...
if df.empty:
    ...
    #Confere Faturamento
    pvt = df.pivot_table(index=['DATREF'], columns=['CODFILEMPEPD'], values=['VLRTOTVNDPPS'], aggfunc=sum)
    ...
    pvtmb = df.pivot_table(index=['DATREF'], columns=['CODFILEMPEPD'], values=['VLRMRGBRT'], aggfunc=sum)
    ...
    pvtmb = df.pivot_table(index=['DATREF'], columns=['CODFILEMPEPD'], values=['VLRPPSMRGCRBVND'], aggfunc=sum)
    pvtmb.columns = pvtmb.columns.get_level_values(1).rename('')
    pvtmb.loc[:,'TOTAL'] = pvtmb.sum(axis=1)
    last_date = pvtmb.iloc[[-1]].index
    pvtmb = pvtmb.append(pd.DataFrame(index=last_date))
    ...
    #Confere Faturamento (Estado BA)
    pvt = df.query('UF=="BA"').pivot_table(index=['DATREF'], columns=['CODFILEMPEPD'], values=['VLRTOTVNDPPS'], aggfunc=sum)
    pvt.columns = pvt.columns.get_level_values(1).rename('')
    pvt.loc[:,'TOTAL'] = pvt.sum(axis=1)
    last_date = pvt.iloc[[-1]].index
    pvt = pvt.append(pd.DataFrame(index=last_date))
    ...
    pvt.iloc[-1:,-1] = 'TOTAL'
    #pvt.to_csv(r'..\ConfereBaseCarga_SNC.csv', sep=";", encoding="iso-8859-1", decimal=",", float_format='%.2f', date_format='%d/%m/%Y', index=False)
    print("Faturamento -- ESTADO BA")
    print(tabulate(pvt, headers='keys', tablefmt='psql', floatfmt=',.2f', showindex=False, numalign='right'),'\n')
    
    #Confere Faturamento (Estado PE)
    pvt = df.query('UF=="PE"').pivot_table(index=['DATREF'], columns=['CODFILEMPEPD'], values=['VLRTOTVNDPPS'], aggfunc=sum)
    pvt.columns = pvt.columns.get_level_values(1).rename('')
    pvt.loc[:,'TOTAL'] = pvt.sum(axis=1)
    last_date = pvt.iloc[[-1]].index
    pvt = pvt.append(pd.DataFrame(index=last_date))
