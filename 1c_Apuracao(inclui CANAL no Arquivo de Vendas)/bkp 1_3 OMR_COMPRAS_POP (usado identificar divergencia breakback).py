...
OMR_FRNUF = pd.read_pickle(r'..\OMR_FRNUF_POP.pkl')
...
df1 = BDCPL.query('MEDIDA=="VLRVNDFATLIQ"').groupby(['DESDRTCLLATU', 'DESCLLCMPATU', 'CODDIVFRN','CODESTUNI'])[['DRIVER']].sum().reset_index()
df2 = OMR_FRNUF.query('MEDIDA=="VLRVNDFATLIQ"').groupby(['DESDRTCLLATU', 'DESCLLCMPATU', 'CODDIVFRN','CODESTUNI'])[['VALOR']].sum().reset_index()
dfdif = pd.merge(df2, df1[['DESDRTCLLATU', 'DESCLLCMPATU', 'CODDIVFRN','CODESTUNI']],  indicator='i', how='outer', on=['DESDRTCLLATU', 'DESCLLCMPATU', 'CODDIVFRN','CODESTUNI']).query('i == "left_only"').drop('i', 1)
dfdif.groupby('DESCLLCMPATU')['VALOR'].sum()
#celula nova = ABB 2-SANDRA
