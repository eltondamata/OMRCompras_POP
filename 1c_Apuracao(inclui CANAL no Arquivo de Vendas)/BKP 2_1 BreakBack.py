...
df_full = pd.read_pickle(r'..\OMR_COMPRAS_POP.pkl')
df_full = pd.melt(
...
df_full = df_full.pivot_table(index=['NOMMES','CODGRPPRD','CODCTGPRD','CODSUBCTGPRD','CODDIVFRN','CODESTUNI','DESTIPCNLVNDOMR','MEDIDA'], values='DRIVER', aggfunc=sum).unstack()
df_full.columns = df_full.columns.get_level_values(1).rename('')
df_full = df_full.reset_index()
df_full['VLRMRGCRB'] = df_full['VLRMRGBRT'] + df_full['VLRCSTMC']

df_full = df_full.query('VLRVNDFATLIQ>0')[['NOMMES','CODGRPPRD','CODCTGPRD','CODSUBCTGPRD','CODDIVFRN','CODESTUNI','DESTIPCNLVNDOMR','VLRVNDFATLIQ','VLRRCTLIQAPU','VLRMRGBRT','VLRMRGCRB']].copy()
df_full.columns = ['NOMMES','CODGRPPRD','CODCTGPRD','CODSUBCTGPRD','CODFRN','CODESTUNI','DESTIPCNLVNDOMR','VLRVNDFATLIQ','VLRRCTLIQAPU','VLRMRGBRT','VLRMRGCRB']
